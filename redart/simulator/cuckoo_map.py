# Adopted from https://github.com/sheldoncooper07/hashing
# -*- coding: utf-8 -*-
...
class CuckooHash(object):
    ...
    def set(self, key, value):
        ...
        return self._set(key, value, increment_nitems=False)

    def _set(self, key, value, increment_nitems=False):
        """Add key value pair and return True on success, False on failure."""
        self._assert_valid_key(key)
        self._assert_valid_value(value)

        if self._is_full() and not self.contains(key):
            return False
