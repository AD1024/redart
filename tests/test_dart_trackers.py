from redart.simulator.dart_sim import RangeTracker, PacketTracker, PacketTrackerEviction, DartSimulator
...
def test_tracker_operations():
    ...
    packet = Packet(src, srcport, dst, dstport, 0, 0, Decimal(0), 1000, PacketType.SEQ)
    ack = Packet(dst, dstport, src, srcport, 1000, 0, Decimal(0.5), 0, PacketType.ACK)
...
def test_flow_insertion_inf_space():
    ...
    trace = [
        ...
        ack_packet(seq_packet(0, Decimal(0.1), 500), Decimal(0.5)),
    ...
    for pid in sim.peer_ids():
        ...
        print("RTT for peer %s <-> %s: %s" % (peer_name[0], peer_name[1], sim.peer_rtt_samples(pid)))
